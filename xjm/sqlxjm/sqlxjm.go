...
func (sjm *sjm) FindJob(name string, asc bool, status ...string) (job *xjm.Job, err error) {
	sqb := sjm.findJobs(name, 0, 1, asc, status...)
	sql, args := sqb.Build()
...
func (sjm *sjm) abortCancelJob(jid int64, status, reason string) error {
	...
	cnt, err := sjm.db.Update(sql, args...)
	if err != nil {
		return err
	}
...
func (sjm *sjm) DeleteJobs(jids ...int64) (jobs int64, logs int64, err error) {
	...
	if logs, err = sjm.db.Update(sql, args...); err != nil {
		return
	}
	...
	jobs, err = sjm.db.Update(sql, args...)
	return
}
